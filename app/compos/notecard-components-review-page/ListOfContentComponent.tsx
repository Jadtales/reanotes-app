...
export default function ListOfContentComponent({
  contentList,
}: {
  contentList: string[];
}) {
  ...
  const windowsWidth = useMediaQuery('(width >= 800px)');
  ...
  return (
    <div
      className={
        windowsWidth
          ? 'listOfContentContainer-largeSize'
          : 'listOfContentContainer-smallSize'
      }
    >
      {!windowsWidth ? (
        <>
          <div className='tableOfContent-phoneSize' ref={TC_phoneSize}>
            {isOpen ? (
              <Image
                src={ShrinkWindowIcon}
                alt={'collapseTheWindow'}
