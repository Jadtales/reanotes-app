...
export default function SharedContentPage(): ReactElement<HTMLDivElement> {
    ...
    const user: string | null | false = getURLQueries.has('user') && getURLQueries.get('user');
    const userId: string | null | false = getURLQueries.has('id') && getURLQueries.get('content-id');


    useEffect(() => {
        async function fetchUserSharedContent(): Promise<void> {
            const getUserSharedContent = await fetch(`http://localhost:3001//shared-content?content-id=${userId}&user=${user}`);
            const data: SharedContentInterface | false = getUserSharedContent.ok && getUserSharedContent.json();
    ...
    return <div className={'sharedContentPageContainer'}
                onMouseEnter={() => setIsStarIconShown(true)}
                onMouseLeave={() => setIsStarIconShown(false)}>

        <div className={'topLayer'}>
            {isStarIconShown && <button className={'starIcon'} onClick={() => setIsSharedContentStared(!isSharedContentStared)}>
                <Image src={isSharedContentStared ? StarIcon : UnStarIcon} alt={'start-unstar-highlight'} width={20}/>
            </button>}
        </div>

        <p className="content">
            {retrievedSharedContent.shared_content}
        </p>

        <div className="username_authorName">
            <cite onClick={handleUserPageRoute} className={'username'}>{capitalize(user)}</cite>
