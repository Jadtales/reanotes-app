...
export default function Notes(): ReactElement {
  ...
  const { tagsElement, noteCardTags, title_description } =
    NoteDescriptionAndTitleComponent();
  ...
  const addNewCards = (): void => {
    ...
    setNoteCards((prevCards) => {
      ...
      const maxKey = Math.max(
        ...prevCards.notecards.map((card) => card.cardKey),
        0
      );
  ...
  const handleCardDeletion = (keyToDelete: number): void => {
    if (noteCards.notecards.length > 4) {
      setNoteCards((prevState) => {
        ...
        const updatedCards = prevState.notecards.filter(
          (card) => card.cardKey !== keyToDelete
        );

        // Sort cards by their current keys to ensure proper ordering
        const sortedCards = [...updatedCards].sort(
          (a, b) => a.cardKey - b.cardKey
        );
  ...
  const handleUpdateCard = (
    ...
    setNoteCards((prevCards) => ({
      ...
      notecards: prevCards.notecards.map(
        (card) =>
          card.cardKey === key
  ...
  return (
    <Fragment>
      <div className='createNotesComponent'>
        <NotecardCreationNavbar isNotecardsPublic={IsNotecardsPublic} />
      ...
      {noteCards.notecards.length === 10 && isSubModalOpen && (
        <SubModalComponent />
      )}
    </Fragment>
  );
